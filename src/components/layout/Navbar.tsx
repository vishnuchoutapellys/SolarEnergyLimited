...
export default function Navbar() {
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between">
            ...
            <Link href="/" className="flex items-center gap-2 sm:gap-3 group shrink-0">
              ...
              <div className="flex flex-col min-w-0">
                <span
                  className={`text-[11px] sm:text-[14px] md:text-[15px] lg:text-[17px] font-extrabold tracking-tight font-heading leading-tight transition-colors duration-300 whitespace-nowrap ${isScrolled ? "text-solar-blue" : "text-white"
                    }`}
                >
                  RUDRAA SOLAR POWER
                </span>
                <span
                  className={`text-[7px] sm:text-[8px] md:text-[9px] lg:text-[10px] font-bold tracking-wider sm:tracking-widest uppercase transition-colors duration-300 whitespace-nowrap ${isScrolled ? "text-solar-orange" : "text-solar-orange-light"
                    }`}
                >
                  PRIVATE LIMITED
                </span>
              ...
              {navLinks.map((link) => {
                ...
                return (
                  ...
                  >
                    {link.name}
                    {isActive && (
                      <span className="absolute bottom-0 left-0 w-full h-0.5 bg-solar-orange rounded-full" />
                    )}
                  </Link>
                );
              })}
            ...
            <div className="flex items-center gap-2.5 md:hidden">
              ...
              <button
                ...
                className={`p-1.5 rounded-lg focus:outline-none focus:ring-2 focus:ring-solar-orange transition-all ${isScrolled ? "text-slate-800 hover:bg-slate-100" : "text-white hover:bg-white/10"
                  }`}
      ...
      {/* ── Mobile Drawer — OUTSIDE header so backdrop-blur doesn't clip it ── */}

      {/* Backdrop overlay */}
      <div
        className={`fixed inset-0 z-[60] bg-slate-900/60 backdrop-blur-sm transition-opacity duration-300 md:hidden ${isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
          }`}
      ...
      {/* Drawer panel */}
      <div
        className={`fixed top-0 right-0 bottom-0 w-[85vw] max-w-sm z-[70] bg-white shadow-2xl transition-transform duration-300 ease-out md:hidden flex flex-col ${isOpen ? "translate-x-0" : "translate-x-full"
          }`}
      >
        {/* Drawer Header — fixed within panel */}
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-2">
            <img
              src="/Logo1.png"
              alt="Rudraa Solar Logo"
              className="h-10 w-auto object-contain"
            />
            <div className="flex flex-col">
              <span className="text-xs font-extrabold text-solar-blue leading-none">RUDRAA SOLAR POWER</span>
